(function() {
    ...
    for (var k = 0; k < input.length; k++) {
        switchLights(input[k], lights);
    }
    
    console.log(countOnLights(lights));
})();

function switchLights(instruction, lights) {
    
    var matches = /(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)/.exec(instruction);
    var startX = parseInt(matches[2]);
    var startY = parseInt(matches[3]);
    var endX = parseInt(matches[4]);
    var endY = parseInt(matches[5]);
    
    switch (matches[1]) {
        case 'turn on':
            setLights(true, startX, startY, endX, endY, lights);
            break;
        case 'turn off':
            setLights(false, startX, startY, endX, endY, lights);
            break;
        case 'toggle':
            setLights(undefined, startX, startY, endX, endY, lights);
            break;
        default:
...
function setLights(value, startX, startY, endX, endY, lights) {
    for (var x = startX; x <= endX; x++) {
        for (var y = startY; y <= endY; y++) {
            lights[x][y] = (value === undefined ? !lights[x][y] : value);
        }
    }
}

function countOnLights(lights) {
    var count = 0;
    for (var i = 0; i < 1000; i++) {
        count += lights[i].filter(function(x) { return x; }).length;
    }
