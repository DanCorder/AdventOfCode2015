(function() {
    ...
    var totalEscapedLiteralLength = 0;
    
    for (var k = 0; k < input.length; k++) {
        totalLiteralLength += literalLength(input[k]);
        totalEscapedLiteralLength += escapedLiteralLength(input[k]);
    }
    
    console.log(totalEscapedLiteralLength - totalLiteralLength);
})();

function literalLength(value) {
    return value.length + 2;
}

function escapedLiteralLength(value) {
    var escaped = value.replace(/\\/g, '\\\\');
    escaped = escaped.replace(/\"/g, '\\"');
    
    return escaped.length + 6;
}
