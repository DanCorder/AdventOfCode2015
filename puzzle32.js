(function() {
    ...
    var splitter = /Sue (\d+)\:(.*)/;
    var propertyParser = / (\w+)\: (\d+),?/g;
    var sueDetails = {};
    
    input.forEach(function(line) {
        var split = splitter.exec(line);
        var sue = {};
        var propertiesString = split[2];
        var propertyMatch;
        while ((propertyMatch = propertyParser.exec(propertiesString)) !== null) {
            sue[propertyMatch[1]] = parseInt(propertyMatch[2]);
        }
        
        sueDetails[split[1]] = sue;
     });
     ...
     for (var sueId in sueDetails) {
         var sue = sueDetails[sueId];
         
         var matches =
            (!sue.hasOwnProperty('children') || sue['children'] === target['children']) &&
            (!sue.hasOwnProperty('cats') || sue['cats'] > target['cats']) &&
            (!sue.hasOwnProperty('samoyeds') || sue['samoyeds'] === target['samoyeds']) &&
            (!sue.hasOwnProperty('pomeranians') || sue['pomeranians'] < target['pomeranians']) &&
            (!sue.hasOwnProperty('akitas') || sue['akitas'] === target['akitas']) &&
            (!sue.hasOwnProperty('vizslas') || sue['vizslas'] === target['vizslas']) &&
            (!sue.hasOwnProperty('goldfish') || sue['goldfish'] < target['goldfish']) &&
            (!sue.hasOwnProperty('trees') || sue['trees'] > target['trees']) &&
            (!sue.hasOwnProperty('cars') || sue['cars'] === target['cars']) &&
            (!sue.hasOwnProperty('perfumes') || sue['perfumes'] === target['perfumes']);
