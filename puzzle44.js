...
function findLowestCost(player, boss, spells, bestSoFar, costSoFar) {
    ...
    playerWithLingeringEffects.HitPoints -= 1; 
    if (playerWithLingeringEffects.HitPoints <= 0) {
        return Number.MAX_SAFE_INTEGER;
    }
    ...
    while (Object.keys(triedSpells).length < 5)  {
        ...
        if (costSoFar + spells[spellName].Cost > bestSoFar) {
            continue;
        }
        ...
        if (bosWithSpellApplied.HitPoints <= 0) {
            // Win
            bestSoFar = Math.min(bestSoFar, costSoFar + spells[spellName].Cost);
            continue;
        }
        ...
        bestSoFar = Math.min(bestSoFar,
            findLowestCost(playerWithSpellApplied, bosWithSpellApplied, spells, bestSoFar, costSoFar + spells[spellName].Cost));
    }
